...
3. *Thinking/reasoning config*: `usage.completion_tokens_details.
   reasoning_tokens` is `0` for the default model both with and
   without an explicit
...
time-to-first-delta floor for that model, reproduced with
...
families (across providers). Closing the gap to <3s would need one of:
...
3. **p50 time-to-first-token badly fails the <3s exit bar, and it is NOT a
   ...
   families (across providers), so it isn't specific to this pipeline's
