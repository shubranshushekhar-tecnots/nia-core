...
just its docs): a provider-style content-block system message with
`cache_control: {type: "ephemeral"}` against the default LLM model
...
alias.** `env.NIA_GATEWAY_MODEL` resolves to the default configured model
through Nia Gateway; its response carries a `canonicalSlug` for that
same default configured model, with a fixed knowledge-cutoff
