...
// see docs/decisions.md's Vault-removal entry.
...
// pre-existing checkDbReachable probe below.
 ...
 * checkDbReachable() exactly (same rationale: every credential resolution
 * here goes through resolveSecret() above, over DATABASE_URL; an
 * unreachable database would otherwise only surface as a generic
 * connection error on the first real request, often minutes into a run).
 */
