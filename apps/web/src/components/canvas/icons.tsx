 ...
 * projects can't be differentiated"). Category/generic fallback icons below
 * are monochrome inline SVGs using `currentColor` (no hardcoded brand hex)
 * so their color comes from the caller (GraphFlowNode.tsx's KIND_COLOR /
 * theme tokens). Connector *brand* icons (CONNECTOR_ICONS) are the
 * exception: they render each vendor's real logo mark with its own fixed
 * brand hex fill (path data + hex from simple-icons, CC0), because a
 * recolored/monochrome version of a trademarked logo isn't recognizable as
 * "the real MySQL/MongoDB/Supabase logo" — that's the whole point of
 * showing one on a connector card. Adding a new connector's real logo is
 * one entry in CONNECTOR_ICONS, not a component change.
 */
export type IconComponent = (props: { size?: number }) => ReactElement;

// ---------- connector brand logos (real marks, keyed by CONNECTOR_MANIFESTS id) ----------
