...
/**
 * Stage 5 production-readiness pass — signs ReadContext (contract.ts), the
 * lighter read-path counterpart to WriteSignaturePayload above, covering
 * /test, /introspect, /execute, /preflight (this worker never calls
 * /invalidate — see apps/api/src/lib/connectorDispatch.ts for that one).
 ...
 * operational difference. This worker only ever signs (never verifies) a
 * ReadContext — verifyReadContext still exists here to keep this file the
 * same shape as its connector-side duplicates, even though nothing calls it.
 */
