 ...
 * touches only that module." Requires requireCookieAuth + attachDb +
 * attachActor to have already run (same precondition every other route in
 * this codebase has for req.withUser/req.actor).
 */
export function getActingUser(req: Request): ActingUser {
  if (!req.withUser || !req.actor) {
